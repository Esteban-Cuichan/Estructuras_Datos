...
Nodo* Dijkstra::obtenerMenorNoVisitado()
{
    ...
    while (actual != nullptr)
    {
        ...
        if (nodo != nullptr && !nodo->estaVisitado())
        {
            if (
                menor == nullptr ||
                nodo->getTiempoAcumulado() <
                menor->getTiempoAcumulado()
            )
            {
                menor = nodo;
            }
...
bool Dijkstra::calcular(int origen, int destino)
{
    ...
    nodoOrigen->setTiempoAcumulado(0.0);
    nodoOrigen->setDistanciaAcumulada(0.0);
    ...
    while (true)
    {
        ...
        // Ya encontramos el destino con su menor costo.
        ...
        while (conexion != nullptr)
        {
            ...
            )
            {
                ...
                if (!vecino->estaVisitado())
                {
                    ...
                    if (
                        nuevoTiempo <
                        vecino->getTiempoAcumulado()
                    )
                    {
                        vecino->setTiempoAcumulado(
                            nuevoTiempo
                        );

                        vecino->setDistanciaAcumulada(
                            nuevaDistancia
                        );
...
void Dijkstra::imprimirRuta(int destino)
{
    ...
    if (
        nodoDestino->getTiempoAcumulado() ==
        numeric_limits<double>::infinity()
    )
    {
    ...
    cout << "\n==============================" << endl;
    cout << "RUTA MAS RAPIDA ENCONTRADA" << endl;
    cout << "==============================" << endl;
