...
void ListaDCE::guardarListaEnArchivo() {
    ...
    if (cabeza != nullptr) {
        ...
        do {
            Turno* t = actual->getTurno();
            if (t != nullptr) { // Añadimos una validación de seguridad
                archivoOut << t->getIDturno() << ","
                           ...
                           // FORZAMOS EL ORDEN AQUÍ INTERCAMBIANDO LAS FUNCIONES:
                           << t->getVehiculo().getColor() << ","  // Escribe "rojo"/"Cafe" en la col 7
                           << t->getVehiculo().getPlaca() << ","  // Escribe la placa real en la col 8
    ...
    do {
        ...
        if (t != nullptr) {
            ...
            cout << "  ID TURNO: " << t->getIDturno() 
                 ...
                 << " | Placa Cifrada: " << digitoLetras << digitoNumeros << endl;
        }
        actual = actual->getSiguiente();
    } while (actual != cabeza);
