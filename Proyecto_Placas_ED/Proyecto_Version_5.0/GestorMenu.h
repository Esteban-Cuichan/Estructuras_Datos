...
#include <ctime>
#include <windows.h>
#include <sstream>  // Para formatear strings limpiamente si es necesario
#include <fstream>  // Para el manejo de archivos de reportes externos
#include "ListaDCE.h"
#include "Persona.h"
#include "Vehiculo.h"
#include "Fecha.h"
...
class GestorMenu {
    private:
        ...
        int leerEntero(const string& mensaje) {
            ...
            while (true) {
                ...
                if (!esNumeroPuro) {
                    cout << "[ERROR] Entrada invalida. Ingrese UNICAMENTE los numeros enteros (no use ':', '-', espacios ni letras).\n" << endl;
                    continue;
                }
    ...
    public:
                ...
                if (tecla == 0 || tecla == 224) {
                    ...
                    switch (opcionActual) {
                        ...
                        case 3: {
                            ...
                            int nh = leerEntero("Nueva Hora (08-16): ");
                            int nm = leerEntero("Nuevos Minutos: ");
        ...
        void ejecutarOrdenamientoTemplates(){
            ...
            while(subMenuActivo){
                ...
                if(tecla == 0 || tecla == 224){
                    ...
                    // Bloque para aislar lógicamente el reporte de la opción 4
                    if(opcionSub == 4) {
                        ...
                        if (sistemaTurnos.getCabeza() == nullptr) {
                            ...
                            for (int i = 0; i < totalTurnos; i++) {
                                ...
                                cout << "  [+] " << f.getDia() << "/" << f.getMes() << "/" << f.getYear() << " - "
                                     << f.getHora() << ":" << minStr << "h - " << placa << endl;

                                // Guardar en archivo si está abierto
                                if (archivoReporte.is_open()) {
                                    archivoReporte << f.getDia() << "/" << f.getMes() << "/" << f.getYear() << " - "
                                                   << f.getHora() << ":" << minStr << "h - " << placa << "\n";
                                }
                            }
                            cout << "========================================================" << endl;
                        ...
                        // Procesar el resto de criterios tradicionales usando tu método por templates
        ...
        void ejecutarAgendamiento() {
            ...
            // ESTA LÍNEA ES CRUCIAL: Limpia el Enter residual que dejó el menú principal
            cin.clear();
            
            cout << "\n=== AGENDAR NUEVO TURNO ===" << endl;
            
            // 1. VALIDACIÓN INDIVIDUAL DEL AÑO
            while (true) {
            ...
            // Bucle principal para permitir regresar si el mes no tiene días hábiles
            while (true) {
                
                // 2. VALIDACIÓN INDIVIDUAL DEL MES
                while (true) {
                ...
                // 3. VALIDACIÓN INDIVIDUAL DEL DIA Y FIN DE SEMANA / FERIADOS
                while (true) {
                    cout << "--- (O si desea cambiar de mes, ingrese 0) ---" << endl;
                    d = leerEntero("Ingrese el dia: ");
                    
                    // Condición de escape para no quedarse atrapado
                    if (d == 0) {
                        regresarAMes = true;
                    ...
                    // VALIDACIÓN INMEDIATA DEL FIN DE SEMANA Y FERIADOS
                    ...
                    break; // Día válido, salimos del bucle del día
                }

                // Si el usuario presionó 0, este break NO se ejecuta y el bucle exterior repite el MES
                if (!regresarAMes) {
                    break; 
                }
            }
            ...
            while (true) {
                hh = leerEntero("Ingrese la hora en formato militar (08-16): ");
                if (hh < 8 || hh > 16) {
                    cout << "[RECHAZADO] El horario de atencion es de 08:00 a 16:00. Ingrese solo las dos cifras de la hora.\n" << endl;
                    continue;
                }
                if (a == anioActual && m == mesActual && d == diaActual && hh < horaActual) {
                    cout << "[RECHAZADO] La hora ingresada ya paso. Hora actual: " << horaActual << "h.\n" << endl;
                    continue;
                }
                break;
            }

            // 5. VALIDACIÓN INDIVIDUAL DE LOS MINUTOS
            while (true) {
                mm = leerEntero("Ingrese los minutos (0-59): ");
                if (mm < 0 || mm > 59) {
                    cout << "[RECHAZADO] Los minutos deben estar entre 0 y 59.\n" << endl;
                    continue;
                }
                if (a == anioActual && m == mesActual && d == diaActual && hh == horaActual && mm <= minActual) {
                    cout << "[RECHAZADO] Los minutos ya pasaron de la hora actual. Hora actual: " << horaActual << ":" << minActual << "h.\n" << endl;
                    continue;
                }
                break;
            }

            // Construcción del objeto definitivo con la hora y minutos verificados
            ...
            // 11. VALIDACIÓN DEL AÑO DEL VEHÍCULO
            while(true) {
            ...
            // Validación de Calendarización ANT según último dígito
            int indicador = -1;
            ...
            // Registro exitoso en la estructura circular en memoria
